...
# Calculate the true total discounted reward
# Take initial action a in initial state s.
def true_q_value(algo: AlgoProtocol, episodes: List[Episode]) -> float:
    for episode in episodes:
        for batch in _make_batches(episode, WINDOW_SIZE, algo.n_frames):
            next_action = algo.predict([batch.next_observations[0]])
            next_value = algo.predict_value(
                [batch.next_observations[0]], next_action
            )
            ...
            q_value = rewards + algo.gamma * cast(np.ndarray, next_value) * mask
    return float(np.mean(q_value))
